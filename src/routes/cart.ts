 ...
 * Cart data stored in Redis with 24-hour expiration
 */
interface CartItem {
    key: string;
    id: string;
    ...
    price: number;
    quantity: number;
    variations: Array<{ attribute: string; value: string }>;
    stock_status: string;
}
...
// Get cart from Redis
async function getCartFromRedis(sessionId: string): Promise<Cart> {
...
async function saveCartToRedis(sessionId: string, cart: Cart): Promise<void> {
    await redis.setex(`cart:${sessionId}`, CART_EXPIRY_SECONDS, JSON.stringify(cart));
}
